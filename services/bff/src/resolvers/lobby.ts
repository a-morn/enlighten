...
export const lobbyMutationResolvers = (redisClient: Redis, pubSub: RedisPubSub) => ({
    joinLobby: (_: unknown, { player: { id, categoryId, name } }: JoinLobbyInput) => {
    ...
    requestGame: async (_: unknown, { gameRequest: { playerRequestId, playerOfferedId, categoryId } }: RequestGameInput, context: Context) => {
        const { currentUser: { playerId } } = context
        if (!isCategoryId(categoryId)) {
            throw new UserInputError('Incorrect category')
        }
    ...
    answerGameRequest: async (_: unknown, { answer: { gameRequestId, accepted } }: AnswerGameRequestInput, { currentUser: { playerId } }: Context) => {
    ...
    deleteGameRequest: async (_: unknown, { gameRequest: { gameRequestId } }: DeleteGameRequestInput, { currentUser: { playerId } }: Context) => {
        const gameRequest = await deleteGameRequest(redisClient, pubSub, playerId, gameRequestId)
