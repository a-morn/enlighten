...
const getGameByPlayerIdInternal = async (redisClient: Redis, playerId: string): Promise<GameSingeplayer | null> => {
...
const updateGameByPlayerId = async (redisClient: Redis, playerId: string, game: GameSingeplayer): Promise<string> => {
  const gameString = JSON.stringify(game);
  return redisClient.set(`singleplayer:games:${playerId}`, gameString);
}


const deleteGameByPlayerId = async (redisClient: Redis, playerId: string): Promise<GameSingeplayer> => {
  ...
  return redisClient
    .del(`singleplayer:games:${playerId}`)
    .then(() => game);
}

const createGame = async (redisClient: Redis, playerId: string, categoryId: CategoryId) => {
  ...
  const updatedGame = await updateQuestionByPlayerId(redisClient, playerId, game)
  return updatedGame
}
...
const updateQuestionByPlayerId = async (redisClient: Redis, playerId: string, game?: GameSingeplayer) => {
  if (isUndefined(game)) {
...
const answerQuestion = async (redisClient: Redis, pubSub: RedisPubSub, playerId: string, questionId: string, answerId: string) => {
  const game = await getGameByPlayerIdInternal(redisClient, playerId)
  if (!game) {
    throw new UserInputError(`Player ${playerId} is not in a game`)
  }
  ...
  const currentQuestion = game
    ...
    .find(({ id }) => id === game.currentQuestionId);
  ...
  setTimeout(async () => {
    const gameFiltered = filterGame(await updateQuestionByPlayerId(redisClient, playerId))
    pubSub.publish(GAME_SINGLEPLAYER, {
      gameSingleplayerSubscription: {
